...
def guidedfilter(im1,im2,r,epsilon): #pour les images en couleurs
    """
    input:
        im1(numpy array): image sur laquelle appliquer le filtre guidé
        im2(numpy array): masque
        r (int): taille des patchs
        epsilon(float): parametre de regularisation
        
    output:
        output(numpy array): image resultante
    """
    n,p  = im2.shape
    im1 = im1/255 #Les pixels doivent prendre des valeurs entre 0 et 1
    im2 = im2/255
    ksize = (2*r+1, 2*r+1)
    ...
    output = (cv2.filter2D(a[:, :, 0], -1, kernel, borderType = cv2.BORDER_ISOLATED)* im1[:, :, 0]
...
def guidedfilter0(im1,im2,r,epsilon): # pour les images en niveau de gris
    ...
    output = (cv2.filter2D(a, -1, kernel, borderType = cv2.BORDER_ISOLATED)* im1
        + cv2.filter2D(b, -1, kernel, borderType = cv2.BORDER_ISOLATED))/W  # Equation (9) de l'article
    return(output)
...
def weightMap(imaList):
    """
    input:
        imaList (liste de numpy arrays): images sur lesquelles calculer les cartes de poids
    
    output:
        sal(liste de numpy arrays): liste des cartes de poids.
    """
...
def fusion(imageList1, imageList2):
    """
    input:
        imageList1 (list d'arrays): liste d'images
        imageList2 (list d'arrays): liste des cartes de poids
        
    output:
        image resultante de la fusion des deux listes
    """
    imageList1, imageList2 = np.array(imageList1),  np.array(imageList2)
    if(imageList1.ndim == 4):
        imageList2 = np.array([imageList2 for i in range(3)])
        imageList2 = np.transpose(imageList2, (1,2,3,0))
    fusion = np.sum(imageList1*imageList2, axis = 0)
    return(fusion)



def execute(folder, fusionname):
    """
    inputs:
        folder (string): direction du dossier contenant les images
        fusionname (string): nom du fichier contenant l'image resultante
    """
    color = 1
    st =os.listdir(folder)# lecture des images à partir d'un dossier
    #st[0] n'est pas un nom d'image donc on l'enlève de la liste à l'étape suivante
    ...
    l = len(imList)
    for i in range(l) :
        imList[i] = cv2.imread(folder+'/'+imList[i]) #lecture des images du dossier
    ...
    if color ==1 :
        for i in range(len(imList)):
            ...
            pic = imList2[i][:,:]
            WB[i] = guidedfilter(imList2[i], weightMapList[i], 23, 0.1)
            print('guidedfilter 2')
            WD[i] = guidedfilter(pic, weightMapList[i], 4, 10**(-7))
    else :
        for i in range(len(imList)):
            print('guidedfilter 1')
            pic = imList2[i][:,:]
